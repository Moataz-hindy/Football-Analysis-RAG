...
class AgentStancePoint(BaseModel):
    ...
    agent_id: str
    round_num: int
    stance_value: float | None = Field(
        ...,
        ge=-1.0,
        le=1.0,
        description="Continuous numeric stance score in [-1.0, 1.0]. Relative to explicitly declared poles; null means unclassified.",
    )
...
class OpinionTrajectoryResult(BaseModel):
    ...
    def get_stance_series(self, agent_id: str) -> list[float | None]:
    ...
    def get_round_stances(self, round_num: int) -> dict[str, float]:
        ...
        for agent_id, points in self.trajectories.items():
            for p in points:
                if p.round_num == round_num and p.stance_value is not None:
...
class RoundAgreement(BaseModel):
    ...
    agreement_score: float | None = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Normalized agreement score in [0.0, 1.0]. 1.0 = identical scores; 0.0 is attainable for two opposing agents, not a general group minimum.",
    )
    mean_distance: float | None = Field(
        ...,
        ge=0.0,
        le=2.0,
...
class DiscussionAgreementResult(BaseModel):
    ...
    mean_discussion_agreement: float | None = Field(default=None, ge=0.0, le=1.0)
    ...
    def get_agreement_series(self) -> list[float | None]:
...
class AgentInfluence(BaseModel):
    ...
    influence_score: float | None = Field(
        default=None,
        description="Observed distance-reduction score in [-1.0, 1.0], or None if insufficient variance.",
    )
    status: str = Field(
        default="valid",
